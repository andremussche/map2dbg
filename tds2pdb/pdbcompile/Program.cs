...
namespace pdbcompile
{
	...
	public class PdbCompiler
	{
		...
		public static void Compile(Stream[] streams, Stream output, uint pageSize)
		{
			...
			// пишем индексы страниц остальных стримов
	...
	class Program
	{
		...
		{
			string[] files = Directory.GetFiles(@"..\..\test");
			Stream[] streams = new Stream[files.Length];
			for (int i = 0; i < files.Length; i++)
				streams[i] = File.OpenRead(files[i]);
			FileStream output = File.OpenWrite("out.pdb");
			uint pageSize = 1024;

			PdbCompiler.Compile(streams, output, pageSize);
		}
	}
}
